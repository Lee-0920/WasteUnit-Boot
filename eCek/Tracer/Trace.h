 ...
 */
#if defined(_CS_TRACE_LEVEL)
#define TRACE_LEVEL(level, fmt, ...)           \
            if ((level) <= g_traceLevel) Console_Out(fmt, ##__VA_ARGS__)
#else
#define TRACE_LEVEL(level, fmt, ...)
#endif
 ...
 *  <p> 使用格式与 printf 相似。
 */
#if defined(_CS_TRACE_LEVEL) && (TRACE_LEVEL_FATAL <= _CS_TRACE_LEVEL)
 ...
 *  <p> 使用格式与 printf 相似。
 */
#if defined(_CS_TRACE_LEVEL) && (TRACE_LEVEL_ERROR <= _CS_TRACE_LEVEL)
 ...
 *  <p> 使用格式与 printf 相似。
 */
#if defined(_CS_TRACE_LEVEL) && (TRACE_LEVEL_WARNING <= _CS_TRACE_LEVEL)
 ...
 *  <p> 使用格式与 printf 相似。
 */
#if defined(_CS_TRACE_LEVEL) && (TRACE_LEVEL_INFO <= _CS_TRACE_LEVEL)
 ...
 *  <p> 使用格式与 printf 相似。
 */
#if defined(_CS_TRACE_LEVEL) && (TRACE_LEVEL_DEBUG <= _CS_TRACE_LEVEL)
 ...
 */
#if defined(_CS_TRACE_LEVEL) && (TRACE_LEVEL_MARK <= _CS_TRACE_LEVEL)
#define TRACE_MARK(fmt, ...)    TRACE_LEVEL(TRACE_LEVEL_MARK, fmt, ##__VA_ARGS__)
#else
#define TRACE_MARK(fmt, ...)
#endif
 ...
 * @note 需要打开宏编译开关 @ref _CS_TRACE_LEVEL 。
 */
#if defined(_CS_TRACE_LEVEL) && (TRACE_LEVEL_CODE <= _CS_TRACE_LEVEL)
#define TRACE_CODE(fmt, ...)    TRACE_LEVEL(TRACE_LEVEL_CODE, fmt, ##__VA_ARGS__)
#else
#define TRACE_CODE(fmt, ...)
#endif
...
/**
 * @brief 输出一个字符串，不接受格式化控制符。
 */
#define Print       Console_OutString
